...
@app_commands.command(name='border', description='Calculate the note border for a chart')
@app_commands.describe(chart_type='Chart type (ST/DX)',
					   diff='Chart difficulty',
					   query='Song name',
					   brk='Amount of low perfect breaks, defaults to 1/10 of the chart\'s break count')
async def border(ctx, chart_type: str, diff: str, query: str, brk: int =None):
	try:
		...
		await ctx.response.send_message(warning_text, embed=emb)
