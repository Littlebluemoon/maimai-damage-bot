...
@app_commands.command(name='mash', description='Can you anmitsu this?')
@app_commands.describe(bpm='BPM of the note pattern', division='Beat division of the note pattern')
...
@app_commands.command(name='hold', description='How long must you hold a note to get Perfect')
@app_commands.describe(bpm='BPM of the note', duration='Duration of the hold note')
async def hold(ctx, bpm: float, duration: str):
    bpm = float(bpm)
    duration_frac = duration.split(':')
    if bpm <= 0:
        await ctx.response.send_message("BPM must be a positive number.")
    else:
        if int(duration_frac[0]) <= 0:
            await ctx.response.send_message("Beat division denominator must be greater than zero.")
        if int(duration_frac[0]) > 1920:
            await ctx.response.send_message("Beat division denominator must be 1920 or less.")
        elif int(duration_frac[1]) < 0:
            await ctx.response.send_message("Beat division numerator cannot be negative.")
        else:
            hold_duration = 240 / bpm * int(duration_frac[1]) / int(duration_frac[0])
            if hold_duration >= 360:
                await ctx.response.send_message("I know, I know, but the game doesn't have a hold that long...")
            else:
    ...
    await ctx.response.send_message(judge_str)