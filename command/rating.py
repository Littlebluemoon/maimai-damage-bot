	...
	if cc < 1 or cc > 15:
		...
		for i in range(len(ACHV)):
			if achv_rate < ACHV[i]:
				...
				break
		await ctx.send(f'''
			A chart with a difficulty of {cc} and a score of {rate}% would award you **{math.floor(min(1005000, achv_rate) * RANK[i] * float(cc) / 10000000)}** rating.''')
